...
import net.teamfruit.bnnwidget.*;
import net.teamfruit.bnnwidget.compat.OpenGL;
import net.teamfruit.bnnwidget.component.*;
...
public class GuiSettings extends WPanel {
    ...
    protected void initWidget() {
        final boolean isUpdateRequired = Config.VERSION.informationUpdateGui.get() && Compat.CompatVersionChecker.isOutdated(Reference.MODID);
                        ...
                        add(new WPanel(new R(Coord.top(hitarea), Coord.bottom(0))) {
                            ...
                            protected void initWidget() {
                                ...
                                add(new MButton(new R(Coord.bottom(5 + updatepanelHeight), Coord.right(5), Coord.width(100), Coord.height(15))) {
                                    @Override
                                    protected boolean onClicked(final @Nonnull WEvent ev, final @Nonnull Area pgp, final @Nonnull Point p, final int button) {
                                        WFrame.displayGuiScreen(new ConfigGui(WFrame.getScreen(ev.owner)));
                                        return true;
                                    }
                                        ...
                                        protected void initWidget() {
                                            add(new WPanel(new R(Coord.left(V.per(hstart, hend, state)), Coord.right(V.per(hstart, hend, state)), Coord.height(V.per(vstart, vend, state)), Coord.ptop(.5f)).child(Coord.ptop(-.5f), Coord.pheight(1f))) {
                                                ...
                                                public boolean mouseClicked(final @Nonnull WEvent ev, final @Nonnull Area pgp, final @Nonnull Point p, final int button) {
                                                    ...
                                                    if (a.pointInside(p)) {
                                                        if (Compat.CompatVersionChecker.isOutdated(Reference.MODID)) {
                                                            String url = Compat.CompatVersionChecker.getResult(Reference.MODID).url;
                                                            if (url != null)
                                                                Client.openURL(url);
                                                        }
                                                        return true;
                                                ...
                                                protected void initWidget() {
                                                    ...
                                                    String update = null;
                                                    if (Compat.CompatVersionChecker.isOutdated(Reference.MODID)) {
                                                        Map<ComparableVersion, String> changes = Compat.CompatVersionChecker.getResult(Reference.MODID).changes;
                                                        if (changes != null)
                                                            update = changes.get(Compat.CompatVersionChecker.getResult(Reference.MODID).target);
                                                    }
