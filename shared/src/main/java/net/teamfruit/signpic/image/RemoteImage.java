...
import net.teamfruit.signpic.entry.content.ContentBlockedException;
import net.teamfruit.signpic.entry.content.ContentLocation;
import net.teamfruit.signpic.entry.content.ContentManager;
import net.teamfruit.signpic.entry.content.RetryCountOverException;
...
import net.teamfruit.signpic.http.download.ContentDownload;
import net.teamfruit.signpic.image.ImageIOLoader.InputFactory.FileInputFactory;
	...
	public void onInit() {
		try {
			...
			if (cachemeta==null||cachemeta.isDirty()||!cachemeta.isAvailable()||!(cacheid!=null&&ContentLocation.cacheLocation(cacheid).exists())) {
				if (Config.CONTENT.contentMaxRetry.get()>0&&this.content.meta.getTryCount()>Config.CONTENT.contentMaxRetry.get())
