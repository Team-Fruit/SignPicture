...
public class EntryId {
	...
	public static @Nonnull EntryId fromChats(final @Nullable ITextComponent[] chats) {
		if (chats==null)
			return blank;
	...
	public static class SignEntryId extends EntryId {
		public static final @Nonnull ITextComponent blankChat = new TextComponentString("");
		public static final @Nonnull SignEntryId blank = new SignEntryId("",
				new String[] { "", "", "", "" }, new ITextComponent[] { blankChat, blankChat, blankChat, blankChat });

		public final @Nonnull String[] strings;
		private @Nullable ITextComponent[] chats;
		...
		protected SignEntryId(final @Nonnull String id, final @Nonnull String[] strings, final @Nonnull ITextComponent[] chats) {
			this(id, strings);
			this.chats = chats;
		}
		...
		protected static @Nonnull SignEntryId fromSignChats(final @Nonnull String[] strings, final @Nonnull ITextComponent[] chats) {
			return new SignEntryId(StringUtils.join(strings), strings, chats);
		}

		public @Nonnull ITextComponent[] chats() {
			if (this.chats!=null)
				return this.chats;
			final ITextComponent[] chats = this.chats = new ITextComponent[4];
			int i = 0;
		...
		public static @Nonnull SignEntryId fromChats(final @Nullable ITextComponent[] chats) {
			if (chats==null)
				return blank;
			...
			int i = 0;
			for (final ITextComponent ch : chs) {
				if (i>=4)
					break;
				if (ch==null) {
					chs[i] = blankChat;
	...
	public void toChats(final @Nullable ITextComponent[] sign) {
		if (sign!=null) {
			final int length = StringUtils.length(this.id);
			for (int i = 0; i<4; i++)
				sign[i] = new TextComponentString(StringUtils.substring(this.id, 15*i, Math.min(15*(i+1), length)));
		}
	}
