	...
	protected void init() {
		add(new WPanel(RArea.diff(0, 0, 0, 0)) {
			...
			public void draw(final WEvent ev, final Area pgp, final Point p, final float frame, final float opacity) {
				...
				glPushMatrix();
				glScalef(a.w(), a.h(), 1f);
				if (GuiImage.this.content.state.getType() == ContentStateType.AVAILABLE) {
				...
				glScalef(.5f, .5f, 1f);
				if (GuiImage.this.content.state.getType() != ContentStateType.AVAILABLE) {
					if (GuiImage.this.content.state.getType() == ContentStateType.ERROR) {
						RenderHelper.startShape();
						glPushMatrix();
					...
					StateRender.drawLoading(GuiImage.this.content.state.progress, GuiImage.this.content.state.getType().circle, GuiImage.this.content.state.getType().speed);
